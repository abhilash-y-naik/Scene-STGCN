...
from dataset_fast import DatasetTrain, DatasetVal
...
from utils import *
from pie_enc_dec import pie_convlstm_encdec
...
class PIEIntent(object):
    ...
    def train(self,
              ...
              optimizer_params={'lr': 0.00001, 'clipvalue': 0.0, 'decay': 0.001},
        ...
        train_dataset = DatasetTrain(train_data, len(train_d['ped_ids']))
        val_dataset = DatasetVal(val_data, len(val_d['ped_ids']))
        ...
        optim = torch.optim.RMSprop(train_model.parameters(),
        ...
        for epoch in range(epochs):
            ...
            for step, (input_enc, input_dec, label) in enumerate(train_loader):
                ...
                y_pred.append(np.where(torch.sigmoid(outputs).data.to('cpu') < 0.5, 0, 1))
                ...
                optim.step()  # (perform optimization step)
    ...
    def test_chunk(self,
        ...
        try:
            ...
            test_model.load_state_dict(torch.load(os.path.join(model_path, 'model.pth')))
        ...
        test_dataset = self.DatasetTest(images, bboxes, ped_ids, decoder_input, output)
