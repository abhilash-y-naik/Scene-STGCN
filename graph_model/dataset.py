...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_images_and_process(self,
        ...
        # decoder_input = torch.zeros((self.seq_len, len(bbox_location_seq[0][0])),  device = torch.device('cuda:0'))
        # graph = torch.zeros((self.seq_len, max_nodes, 512, 7, 7), device = torch.device('cuda:0'))
        ...
        # adj_matrix = torch.zeros((max_nodes, max_nodes), device=torch.device('cuda:0'))
        for s in range(self.seq_len):
            ...
            img_cp_p = img_centre_seq[s][0]
            adj_matrix[0, 0] = 2
            # decoder_input[s, :] = torch.from_numpy(np.squeeze(np.asarray(bbox_location[0])))
        ...
        adj_matrix[:, :] = self.normalize_undigraph(adj_matrix[:, :])
            # print(adj_matrix[s,:,:])
        # print(adj_matrix)
        if visualise:
...
class Dataset_test(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        train_data = torch.from_numpy(graph), torch.from_numpy(adj_matrix), torch.from_numpy(self.track['output'][index][0])
        return train_data
